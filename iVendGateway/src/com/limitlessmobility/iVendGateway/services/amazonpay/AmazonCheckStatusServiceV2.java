...
public class AmazonCheckStatusServiceV2 {
	
	private static final Logger logger = Logger.getLogger(AmazonCheckStatusServiceV2.class);
	...
	public  String getAmazonTransactionstatus(@RequestBody CheckStatusInitialData checkStatusInitialData) throws JSONException{
		...
		try {
			...
			if (checkStatusInitialData.getPosId() != null) {
				...
				System.out.println("AmazonPay terminalId "
				        + checkStatusInitialData.getPosId());
				if (checkStatusInitialData.getPspId() != null) {
					pspId = checkStatusInitialData.getPspId();
					System.out.println("AmazonPay AppId "
					        + checkStatusInitialData.getPspId());
					int operatorId = commonCredentialDao
					        .getOperatorId(terminalId, pspId);

					operatorDetail = commonCredentialDao
					        .getPspConfigDetail(operatorId, pspId);
					...
					System.out.println("merchantKey(accessKey) " + accessKeyId);
					merchantKey = operatorDetail.getMerchantKey();
					System.out.println("PspMguid(secretKey) " + merchantKey);
		   ...
		   if (checkStatusInitialData.getPosId()!=null) {
	        ...
	        requestBody.put("transactionIdType",transactionIdType);
