...
import com.limitlessmobil.ivendgateway.util.HexadecimaToInteger;
...
import com.limitlessmobility.iVendGateway.dao.PayTMDao;
import com.limitlessmobility.iVendGateway.dao.PayTMDaoImpl;
...
public class PayTmQRService {
	...
	/*
	 * This Method is used for creating PayTM QR
	 */
	@RequestMapping(value = "/getqr", method = RequestMethod.POST)
	@ResponseBody
	public String createQRCode(@RequestBody String requestQR) throws Exception {
		System.out.println("Calling PayTM Dynamic QR (createQRCode Method)...START");
		System.out.println("request from common method..."+requestQR);
...
//		logger.info("merchantGuid..ae322099-8e13-4245-a425-fd7daba6865f");
		...
		try {
			...
			if(jsonObj.getString("posId")!=null) {
				terminalId= jsonObj.getString("posId");
				if(jsonObj.getString("pspId")!=null) {
					pspId=jsonObj.getString("pspId");
	        ...
	        paytmQRRequestType.setAmount(jsonObj.getString("amount"));
	        ...
	        paymentInitiation.setMerchantId(mid);
	        paymentInitiation.setComments(paytmFinalQRRequest.getOperationType());
	        paymentInitiation.setAppId(jsonObj.getString("appId"));
...
//			String CHECKSUMHASH = CheckSumServiceHelper.getCheckSumServiceHelper().genrateCheckSum("TC#1Bkm&PWDv3Vz!", urlParameters);
			...
			String CHECKSUMHASH = checksumHelper.genrateCheckSum(cdao.getPspMerchantKey(pspId,terminalId), urlParameters);
			URL url = new URL("https://wallet.paytm.in/wallet-merchant/v2/createQRCode");
			HttpURLConnection connection = null;
			connection = (HttpURLConnection)url.openConnection();
			connection.setRequestMethod("POST");
			
			connection.setRequestProperty("mid",mid.trim());
