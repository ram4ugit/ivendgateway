...
@Controller
@RequestMapping("/v1/zeta")
public class ZetaRefundService {
		...
		try {
			if (isConnected) {
				...
				try {
					...
					String querry22 = "SELECT psp_merchant_key FROM psp_merchant WHERE psp_id='zeta'";
					...
					while (resultSet22.next()) {
						...
						apiKey = resultSet22.getString("psp_merchant_key").trim();
