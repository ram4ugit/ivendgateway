...
public class ZetaCallbackInitiationDaoImpl implements ZetaCallbackInitiationDao {
		...
		try {
			if (isConnected) {
				...
				if (resultSettime.next()) {
					...
					System.out.println("transaction daoimpl1: " + sqlDate);

					sqlDate = resultSettime.getString(1);

					System.out.println(" transaction daoimpl2: " + sqlDate);
				...
				System.out.println("psp trn id: " + callbackTransactions.getPspTransactionId());
				...
				if (isInsert > 0) {
					...
					System.out.println("pspId: " + callbackTransactions.getPspId());

					String querry = "SELECT psp_transaction_id FROM payment_initiation  WHERE psp_id= '"
							+ callbackTransactions.getPspId()
							+ "' AND auth_date=(SELECT MAX(auth_date) AS TIMESTAMP FROM payment_initiation ORDER BY TIMESTAMP DESC) AND Id = (SELECT MAX(Id)FROM payment_initiation WHERE payment_stage='qrscanned')";
					...
					while (resultSet2.next()) {
						...
						pspTrId = resultSet2.getString("psp_transaction_id");

					}

					System.out.println("pspTrId: " + pspTrId);
					...
					jsonObject.put("purchaseTransactionId", pspTrId);
					...
					try {
						...
						// Dynamically get key using psp id from pspmrchnt

						String querry22 = "SELECT psp_merchant_key FROM psp_merchant WHERE psp_id='"
								+ callbackTransactions.getPspId() + "'";
						...
						try {
							...
							paymentTransaction.setPspId(callbackTransactions.getPspId());
							...
							paymentTransaction
									.setPspTransactionId(zetaCallbackTransactions.getMetaInfo().getPaymentReceiptId());
							...
							ZetaCallbackTransactionDao zetaCallbackTransactionDao = new ZetaCallbackTransactionDaoImpl();

							zetaCallbackTransactionDao.saveTransaction(paymentTransaction,zetaCallbackTransactions);
						...
						System.out.println("Exception in CallbackInitiationDaoImpl Service class.." + e);
						e.printStackTrace();
					}
	...
	@Override
	public boolean saveTransactionV2(PaymentTransaction callbackTransactions,
	ZetaCallbackTransactions zetaCallbackTransactions) {
	...
	int operatorId = commonCredentialDao.getOperatorId(callbackTransactions.getTerminalId(), callbackTransactions.getPspId());

	operatorDetail = commonCredentialDao.getPspConfigDetail(operatorId, callbackTransactions.getPspId());


	walletId = operatorDetail.getPspMguid();
	...
	System.out.println("apiKey: " + walletKey);

	conn.setRequestProperty("apiKey", walletKey);
