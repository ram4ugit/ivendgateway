...
public class ZetaCallbackTransactionDaoImpl implements ZetaCallbackTransactionDao {
		...
		try {
			if (isConnected) {
				...
				String query = "INSERT INTO payment_transactions"
						...
						+ "status,comments,status_code,status_msg,refund_amount,stage,order_id,merchant_order_id,service_type,app_id,psp_merchantid,transaction_type) VALUES"
						+ "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				preparedStatement.setString(28, zetaCallbackTransactions.getMetaInfo().getIfi());
